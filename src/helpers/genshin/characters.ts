...
function getEmbed(id: string,
    index: number,): { embed: MessageEmbed; buildNames: string[] } {
    ...
    embed.setAuthor(`${character.name}`, `https://paimon.moe/images/characters/${id}.png`, `https://paimon.moe/characters/${id}`,);
    embed.setColor(colors[character.element.id]);
    embed.setTitle(`${title}${data.recommended ? ' 👍' : ''}`);
    if (data.note !== undefined) {
        embed.setDescription(data.note.replace(/<b>/g, '**').replace(/<\/b>/g, '**'),);
    }
    embed.addField('MAIN STATS', `**Sands:** ${data.mainStats.sands}\n**Goblet:** ${data.mainStats.goblet}\n**Circlet:** ${data.mainStats.circlet}`, true,);
    embed.addField('SUB STATS', data.subStats.map((e, i) => `**${i + 1}.** ${e}`).join('\n'), true,);
    embed.addField('TALENT PRIORITY', data.talent.map((e, i) => `**${i + 1}.** ${e}`).join('\n'), true,);
    embed.addField('WEAPON', data.weapons
        .map((e, i) => {
            let refine = ' ';
            let stack = '';
        ...
        .join('\n'), true,);
    embed.addField('ARTIFACT', data.artifacts
        .map((e, i) => {
            let str;
            if (e.length > 2) {
                str =
...
export async function generateBuildEmbed(id: string,
    ...
    if (buildNames.length > 1) {
        const buttons = buildNames.map((e, i) =>
            new MessageButton({
          ...
          void (message as Message).edit({
              ...
              components: [
                  new MessageActionRow().addComponents(new MessageButton({
                      ...
                      label: `type %genshin ${characters[id].name} to refresh`,
                      style: 'SECONDARY',
                      disabled: true,
                  }),),
              ],
